...
class GrayEdgeImpl : public GrayEdgeWB
{
    int order;
    int norm;
    int sigma;
...
public:
    ...
    GrayEdgeImpl(int order_, int norm_, int sigma_)
        : order(order_), norm(norm_), sigma(sigma_) {}

    void setSigma(int sigma_) { sigma = sigma_; }
    void setMinkNorm(int norm_) { norm = norm_; }
    void setDiffOrder(int order_) { order = order_; }

    int getSigma() const { return sigma; }
    int getMinkNorm() const { return norm; }
    int getDiffOrder() const { return order; }
    ...
    void balanceWhite(InputArray _src, OutputArray _dst)
    {
        ...
        general_cc(img,white_R,white_G,white_B,output);
...
private:
    ...
    void gDer(Mat &f, int iorder, int jorder, Mat &H)
    {
    ...
    void normDerivative(Mat &in, Mat &Rw, Mat &Gw, Mat &Bw)
    {
        int rows = in.rows;
        int cols = in.cols;
        ...
        if(order == 1)
        {
            ...
            gDer(channels[2],1,0,Rx);
            gDer(channels[2],0,1,Ry);
            pow(Rx,2,temp1);
            pow(Ry,2,temp2);
            sqrt(temp1 + temp2,Rw);

            gDer(channels[1],1,0,Gx);
            gDer(channels[1],0,1,Gy);
            pow(Gx,2,temp1);
            pow(Gy,2,temp2);
            sqrt(temp1 + temp2,Gw);

            gDer(channels[0],1,0,Bx);
            gDer(channels[0],0,1,By);
            pow(Bx,2,temp1);
            pow(By,2,temp2);
            sqrt(temp1+temp2,Bw);
        }
        if(order == 2)
        {
            ...
            gDer(channels[2],2,0,Rxx);
            gDer(channels[2],0,2,Ryy);
            gDer(channels[2],1,1,Rxy);
            sqrt(Rxx.mul(Rxx) + 4*Rxy.mul(Rxy) + Ryy.mul(Ryy),Rw);

            gDer(channels[1],2,0,Gxx);
            gDer(channels[1],0,2,Gyy);
            gDer(channels[1],1,1,Gxy);
            sqrt(Gxx.mul(Gxx) + 4*Gxy.mul(Gxy) + Gyy.mul(Gyy),Gw);

            gDer(channels[2],2,0,Bxx);
            gDer(channels[2],0,2,Byy);
            gDer(channels[2],1,1,Bxy);
            sqrt(Bxx.mul(Bxx) + 4*Bxy.mul(Bxy) + Byy.mul(Byy),Bw);
        }
    }

    void general_cc(Mat &I, double &white_R, double &white_G, double &white_B, Mat &output)
    {
        int rows = I.rows;
        int cols = I.cols;
        ...
        if(order == 0)
        {
            if(sigma!=0)
            {
                gDer(planes[2],0,0,planes[2]);
                gDer(planes[1],0,0,planes[1]);
                gDer(planes[0],0,0,planes[0]);
            }
        }

        if(order > 0)
        {
            normDerivative(I,channels[2],channels[1],channels[0]);
            merge(channels,I);
        }
...
cv::Ptr<cv::xphoto::GrayEdgeWB> cv::xphoto::createGrayEdge(int mode)
{
    switch (mode)
    {
    case cv::xphoto::GrayEdgeWB::GrayWorld: return makePtr<GrayEdgeImpl>(0, 1, 0);
    case cv::xphoto::GrayEdgeWB::MaxRGB: return makePtr<GrayEdgeImpl>(0, -1, 0);
    case cv::xphoto::GrayEdgeWB::ShadesOfGray: return makePtr<GrayEdgeImpl>(0, 5, 0);
    case cv::xphoto::GrayEdgeWB::GeneralGrayWorld: return makePtr<GrayEdgeImpl>(0, 5, 2);
    case cv::xphoto::GrayEdgeWB::GrayEdge: return makePtr<GrayEdgeImpl>(1, 5, 2);
    case cv::xphoto::GrayEdgeWB::GrayEdge2: return makePtr<GrayEdgeImpl>(2, 5, 2);
    default: CV_ErrorNoReturn(cv::Error::StsBadArg, "Invalid mode passed into createGrayEdge");
    }
}
