...
void Intrinsic::compute(Mat &rgbIm, Mat &rgbIm2, Mat &weight, Mat &ref, Mat &shade, int iterNum, float rho)
{
    ...
    for(int k = 0;k<iterNum;k++)
    {
        ...
        reduce(temp1,temp1,1,REDUCE_SUM);
...
void Intrinsic::decompose(Mat &rgbIm, Mat &ref, Mat &shade, int wd, int iterNum, float rho)
{
    ...
    for(int j = 0;j<m;j++)
    {
        for(int i = 0;i<n;i++)
        {
            ...
            Mat cvals;
            reduce(rep_mat,cvals,1,REDUCE_SUM);
...
void cv::xphoto::intrinsicDecompose(InputArray _src, OutputArray _ref, OutputArray _shade, int window, int no_of_iter, float rho)
{
     Mat I = _src.getMat();
